...
$('#france-map .land').hover(function(e) {
    ...
    let titleText = "";
    if ("code" in data) {
        titleText = `${data.name} : ${data.projects_count} projets financés`;
    } else {
...
$('#france-map .land').click(function() {
    ...
    let data = get_department_data(selected_path);
    if ("code" in data) {
        let new_url = window.location.origin + "/projets/projets-subventionnes/resultats/?project_perimeter=" + data.id;
