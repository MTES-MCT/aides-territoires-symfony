...
#[AsCommand(name: 'at:import:log_aid_search_category', description: 'Import log aid search category')]
class LogAidSearchCategoryImportCommand extends ImportCommand
{
    protected string $commandTextStart = '>Import log aid search category';
    protected string $commandTextEnd = '<Import log aid search category';

    protected function import($input, $output)
    {
        // ==================================================================
        // LOG AID SEARCH CATEGORY
        // ==================================================================

        $io = new SymfonyStyle($input, $output);
        $io->info('LOG AID SEARCH CATEGORY');

        // fichier
        $filePath = $this->findCsvFile('stats_aidsearchevent_categories_');
        if (!$filePath) {
        ...
        $sqlBase = "INSERT INTO `log_aid_search_category`
        (
        log_aid_search_id,
        category_id
        )
        ...
        if (($handle = fopen($filePath, "r")) !== FALSE) {
            while (($raw_string = fgets($handle)) !== false) {
                ...
                $sqlParams['category_id'.$rowNumber] = (int) $data[2];
