...
#[AsCommand(name: 'at:import:log_aid_view', description: 'Import log aid view')]
class LogAidViewImportCommand extends ImportCommand
{
    protected string $commandTextStart = '>Import log aid view';
    protected string $commandTextEnd = '<Import log aid view';
                ...
                if (is_array($organizationTypeSlugs)) {
                    ...
                    foreach ($organizationTypeSlugs as $organizationTypeSlug) {
                        if (isset($organizationTypeIdBySlugs[$this->redoSlug($organizationTypeSlug)])) {
                            $sql2 .= "
                            (
                                :log_aid_view_id".$rowNumber."_".$i.",
                                :organization_type_id".$rowNumber."_".$i."
                            ),";

                            $sqlParams2['log_aid_view_id'.$rowNumber."_".$i] = (int) $cells[0]->getValue();
