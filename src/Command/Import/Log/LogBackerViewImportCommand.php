...
#[AsCommand(name: 'at:import:log_backer_view', description: 'Import log backer view')]
class LogBackerViewImportCommand extends ImportCommand
{
    protected string $commandTextStart = '>Import log backer view';
    protected string $commandTextEnd = '<Import log backer view';
    ...
    protected function import($input, $output)
    {
        ...
        $io->info('LOG BACKER VIEW');

        // fichier
        $filePath = $this->findCsvFile('stats_backerviewevent_');
        if (!$filePath) {
        ...
        $sqlBase = "INSERT INTO `log_backer_view`
        (
        backer_id,
        organization_id,
        user_id,
        ...
        if (($handle = fopen($filePath, "r")) !== FALSE) {
            while (($raw_string = fgets($handle)) !== false) {
                ...
                $sqlParams['backer_id'.$rowNumber] = $this->intOrNull((string) $data[3]);
