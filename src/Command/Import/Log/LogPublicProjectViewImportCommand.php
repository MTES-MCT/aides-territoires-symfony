...
#[AsCommand(name: 'at:import:log_public_project_view', description: 'Import log public project view')]
class LogPublicProjectViewImportCommand extends ImportCommand
{
    protected string $commandTextStart = '>Import log public project view';
    protected string $commandTextEnd = '<Import log public project view';

    protected function import($input, $output)
    {
        // ==================================================================
        // LOG PUBLIC PROJECT VIEW
        // ==================================================================

        $io = new SymfonyStyle($input, $output);
        $io->info('LOG PUBLIC PROJECT VIEW');

        // fichier
        $filePath = $this->findCsvFile('stats_publicprojectviewevent_');
        if (!$filePath) {
        ...
        $sqlBase = "INSERT INTO `log_public_project_view`
        (
        organization_id,
        project_id,
        user_id,
        ...
        if (($handle = fopen($filePath, "r")) !== FALSE) {
            while (($raw_string = fgets($handle)) !== false) {
                ...
                $sqlParams['project_id'.$rowNumber] = $this->intOrNull((string) $data[3]);
