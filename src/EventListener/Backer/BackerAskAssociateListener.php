                ...
                foreach ($entity->getBacker()->getOrganizations() as $organization) {
                    foreach ($organization->getBeneficiairies() as $beneficiairy) {
                        $this->notificationService->addNotification(
                            $beneficiairy,
                            'Association de la structure '.$organization->getName(). ' avec le porteur d\'aides '.$entity->getBacker()->getName(),
                            'La fiche du porteur d\'aides '.$entity->getBacker()->getName().' a été associée avec la structure '.$organization->getName().'. Vous pouvez la consulter en cliquant sur <a href="'.$backerUrl.'" title="Voir la fiche du porteur d\'aides">ce lien</a>.',
                        );
                    }
                }
